 ...
 * 2. 修改类名,免得经常和org.junit.Assert冲突.
 *
 * 代码示例:
 *
 * <pre class="code">
 * AssertUtils.hasText(uName, new IllegalBizArgumentsException(ErrorCode.USERNAME_ERROR));
 * </pre>
 *
 * @author linlun
 * @version 1.0
...
public abstract class AssertUtils {
    ...
    public static void isNotBlank(String str) {
        isNotBlank(str, "字符串不能为空");
    }
    /**
     * @param str
     * 字符串不为空，为空抛出带有message信息的异常，
     */
    ...
    /**
     * @param str
     * 字符串不为空，为空抛出RuntimeException异常，
     */
    public static void isNotBlank(String str, RuntimeException throwIfAssertFail) {
        if (isBlank(str)) {
            throw throwIfAssertFail;
        }
    }
    ...
    public static void notEmpty(Map<Object, Object> map, RuntimeException throwIfAssertFail) {
        if (CollectionUtils.isEmpty(map)) {
            throw throwIfAssertFail;
        }
    }
